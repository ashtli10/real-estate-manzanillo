...
async function deductCredits(userId: string, amount: number): Promise<boolean> {
  ...
  const { error: txError } = await supabaseAdmin
    ...
    .insert({
      user_id: userId,
      amount: -amount,
      transaction_type: 'ai_video_render',
      service: 'ai_video_generator',
      description: null,
    });
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Parse request body
    const body: ApproveScriptRequest = req.body;
    ...
    if (jobData.status !== 'script_ready') {
      return res.status(400).json({ error: 'Job is not in script_ready state' });
    }
    ...
    try {
      ...
      if (!webhookResponse.ok) {
        // Mark job as failed (no additional credit refund - already charged for video)
        await supabaseAdmin
          .from('video_generation_jobs')
          .update({
            status: 'failed',
        ...
        await refundCredits(userId, GENERATE_VIDEO_CREDIT_COST);
