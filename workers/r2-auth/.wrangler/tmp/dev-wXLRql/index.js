...
// .wrangler/tmp/bundle-ochFaq/checked-fetch.js
...
// .wrangler/tmp/bundle-ochFaq/strip-cf-connecting-ip-header.js
...
function getCorsHeaders(request, env) {
  ...
  const isAllowed = env.ENVIRONMENT === "development" || ALLOWED_ORIGINS.includes(origin) || origin.startsWith("http://localhost:");
  return {
...
function validatePathAccess(path, userId) {
...
__name(validatePathAccess, "validatePathAccess");
async function handleUpload(request, path, env, corsHeaders) {
  const contentType = request.headers.get("Content-Type") || "application/octet-stream";
  const contentLength = request.headers.get("Content-Length");
  const maxSizeBytes = path.includes("/videos/") ? 50 * 1024 * 1024 : 5 * 1024 * 1024;
...
async function handleBatchDelete(request, userId, env, corsHeaders) {
  try {
...
__name(handleBatchDelete, "handleBatchDelete");
async function handleList(request, userId, env, corsHeaders) {
  try {
    const body = await request.json();
    if (!body.prefix || typeof body.prefix !== "string") {
      return new Response(JSON.stringify({
...
// .wrangler/tmp/bundle-ochFaq/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ochFaq/middleware-loader.entry.ts
