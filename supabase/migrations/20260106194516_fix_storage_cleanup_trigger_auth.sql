-- Fix storage cleanup triggers to use service role key instead of JWT claims
-- The JWT claims are not available in AFTER DELETE trigger context

-- First, create a helper function to get the service role key from vault
-- Note: You must manually add the service role key to vault after applying this migration:
-- 
...
-- Update trigger_cleanup_draft_files to use service role key
...
-- Update trigger_cleanup_property_files to use service role key
...
-- Update trigger_cleanup_user_files to use service role key
...
-- Update trigger_cleanup_video_job_files to use service role key
